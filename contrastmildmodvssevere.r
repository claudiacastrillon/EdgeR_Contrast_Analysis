...
data <- read_excel("/Users/claudiacastrillonalvarez/Downloads/mergedmicros.xlsx")
...
# Define conditions
conditions <- factor(c(rep("mildmod", 20), rep("severe", 10)))

# Calculate mean expressions
mean_mildmod <- rowMeans(data[, 1:20])
mean_severe <- rowMeans(data[, 21:30])

# Custom fold change calculation
fold_change <- ifelse(mean_mildmod >= mean_severe, 
                      mean_mildmod / (mean_severe + 0.1), 
                      - (mean_severe + 0.1) / mean_mildmod)
...
# Define contrasts
contrast.matrix <- makeContrasts(ModerateVsSevere = conditionssevere, levels = design)
...
file_path <- "/Users/claudiacastrillonalvarez/Downloads/comparacion_CPM_mildmoderate_vs_severe.xlsx"
