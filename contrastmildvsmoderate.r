...
data <- read_excel("/Users/claudiacastrillonalvarez/Downloads/expression_browsermildvsmod.xlsx")
...
# Define conditions
conditions <- factor(c(rep("mild", 10), rep("moderate", 10)))

# Calculate mean expressions
mean_mild <- rowMeans(data[, 1:10])
mean_moderate <- rowMeans(data[, 11:20])

# Custom fold change calculation
fold_change <- ifelse(mean_mild >= mean_moderate, 
                      mean_mild / (mean_moderate + 0.1), 
                      - (mean_moderate + 0.1) / mean_mild)
...
# Define contrasts
contrast.matrix <- makeContrasts(MildVsModerate = conditionsmoderate, levels = design)
...
file_path <- "/Users/claudiacastrillonalvarez/Downloads/comparacion_CPM_mild_vs_moderate.xlsx"
